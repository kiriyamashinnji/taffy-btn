...
async function initialize() {
  ...
  overview.forEach((l) => {
    l.button_list.forEach((l) => {
      soundURLs.push(l.voice_url);
    });
  });

  player.soundList = soundURLs;
...
function initializeSidebarBackground() {
  // set width
  $("#sidebar-mask-container").css({
    width: String($("#sidebar-wrapper").width() + 10),
  });
  $("#sidebar-mask-container").css({
    height: String($("#sidebar-wrapper").height()),
  });
  $("#sidebar-mask").css({ width: String($("#sidebar-wrapper").width()) });
  let top_px =
    (-$("#sidebar-mask").width() / 356) * originalMid +
...
function initializeSidebar(text, idx) {
  $("#categories").append(
    '<a class="list-group-item li-p" id="sidebar-' +
      String(idx) +
      '"href="#!">' +
